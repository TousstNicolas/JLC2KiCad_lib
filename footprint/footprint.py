...
def create_footprint(footprint_component_uuid, component_id, footprint_lib, output_dir, model_path_relative):
	...
	class footprint_info():
		def __init__(self, footprint_name, output_dir, footprint_lib):
			self.max_X, self.max_Y, self.min_X, self.min_Y = -10000, -10000, 10000, 10000 # I will be using these to calculate the bounding box because the node.calculateBoundingBox() methode does not seems to work for me 
	...
	kicad_mod.append(Text(type='reference', 
						  text='REF**', 
						  at=[(footprint_info.min_X + footprint_info.max_X)/2, 
